...
subString start end str | end < 0 = "" | start < 0 && end >= 0 = take (end + 1) str | otherwise = take (end - start + 1) (drop start str)
...
-}
lowerAndGreater :: (Show a, Ord a, Foldable t) => a -> t a -> [Char]
